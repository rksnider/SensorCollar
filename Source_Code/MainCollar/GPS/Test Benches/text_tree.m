...
%TEXT_TREE    Convert a vector of text strings into a tree of character nodes.
...
%   Sort the table by string value and determine the dimensions of the result.
...
while (tos > 0)
  ...
  %   Take the last list entry offset from the top-of-stack and advance to the
  %   next node.
  ...
  %   Add the characters to the tree for strings that match the current prefix.
  ...
  for k = 1 : length (sorted_strs)
    ...
    if (length (str) > preflen)
      if (min (str (1:preflen) == prefix))
        ...
        %   The offset for this character's list of following characters will
        %   be updated each time a character is added to the current list.
  ...
  %   with it.  Return to the previous list level by popping this list off the
  %   stack.
