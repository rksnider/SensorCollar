...
if {[get_collection_size [get_nodes $clk_target]] > 0} {
  ...
  set sd_clk_ext_name     sdclk_dir_ext
  set sd_clk_400_ext_name   sd_clk_400_ext

  #Create Virtual clocks for input_delay constraints. 
  create_clock -period $sdclk_period -name $sd_clk_ext_name
  create_clock -period $sdclk_400_period -name $sd_clk_400_ext_name
                         
  #A mutliplexer at microsd_controller_inner requires separation between fast and 400k sd clocks. 
  set_clock_groups -exclusive -group [list $sdclk_dir $sysclk_clock $sd_clk_ext_name]  -group [list $sdclk_dir_400 $sdclk_400 $sd_clk_400_ext_name]
                        
  #Use the set_keyvalue so we can get at these from SDcard_dev.sdc
  set_keyvalue                  "$sdclk_port" [list "$sysclk_clock" "$sdclk_400" "$sdclk_dir" "$sdclk_dir_400" "$sd_clk_ext_name" "$sd_clk_400_ext_name"]
} else {
